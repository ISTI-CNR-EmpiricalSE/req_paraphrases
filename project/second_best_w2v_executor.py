...
def second_best_w2v_executor_func(filename, parameters_list, output_dict):
    """
    ...
    is calculated comparing the synonym with the original terms of the phrase and their synonyms by the similarity()
    method of KeyedVectors, summing single similarities and dividing the result by number of comparisons.
    Otherwise, if the syn_vs_term flag is set to True the score of a synonym is calculated comparing the synonym with
    the original terms of the phrase by the similarity() method of KeyedVectors, summing single similarities
    ...
    The i-th output phrase is the original phrase with i terms replaced by their second best synonyms.
    ...
    (if n_max == 100 last phrase is the original phrase with all approved terms replaced by their second best synonym)
    ...
    for phrase in phrases:
        ...
        best_synset_dict = {}  # {token_1: [best_syn_1, second_best_syn_1], token_2: [best_syn_1, second_best_syn_2]...}
                        ...
                        if word_approved(compare_token):
                            ...
                            if syn_vs_synsets:
                                ...
                                for compare_synset in compare_synsets:
                                    ...
                                    # synonym that must obtain the scoring, in non synset format
                                    syn_name_final = from_synset_to_string(synset)

                                    num_cmp = num_cmp + 1

                                    try:
                                        # comparing syn_name_final with compare_syn_name_final
                                        sim = w2v_model.similarity(syn_name_final, compare_syn_name_final)
                                    except KeyError:
                                        sim = 0
                                    if sim:
                                ...
                                try:
                                    # comparing syn_name_finale with compare_token.text.lower()
                                    sim = w2v_model.similarity(syn_name_final, compare_token.text.lower())
                                except KeyError:
                                    sim = 0
                                if sim:
                                    scoring = scoring + abs(sim)
