...
def hyper_w2v_executor_func(filename, parameters_list, output_dict):
    """
    Function that replaces terms with their best hypernym according to the context.
    The best hypernym is the hypernym with the higher score.
    If the syn_vs_synsets flag in the list of parameters configured by the user is set to True the score of a hypernym
    is calculated comparing the hypernym with the original terms of the phrase and their synonyms by the similarity()
    method of KeyedVectors, summing single similarities and dividing the result by the number of comparisons.
    Otherwise, if the syn_vs_term flag is set to True the score of a hypernym is calculated comparing the synonym with
    ...
    If no hypernym score is greater than 0.1, then no hypernym will be selected to be the best hypernym.
    If first best hypernym is not found (no hypernym with a score > 0.1), the word is not replaced.
    The i-th output phrase is the original phrase with i terms replaced by their best hypernyms.
    The configurable parameter n_max indicates the percentage of replaced terms.
    (if n_max == 100 the last phrase is the original phrase with all approved terms replaced by their best hypernym)
        ...
        for token in tokens:
            ...
            if word_approved(token):
                ...
                for a in synset_dict[token.text]:
                    synsets.append(a)
                # for each token i have a dictionary to save the scoring of each of his hyp
                                ...
                                if word_approved(compare_token):
                                    ...
                                    if syn_vs_synsets:
                                        ...
                                        for compare_synset in compare_synsets:
                                            ...
                                            hyper_name_final = from_synset_to_string(hyper)

                                            num_cmp = num_cmp + 1

                                            try:
                                                # comparing hyper_name_final with compare_syn_name_final
                                                sim = w2v_model.similarity(hyper_name_final, compare_syn_name_final)
                                            except KeyError:
                                                sim = 0
                                            if sim:
                                        ...
                                        try:
                                            # comparing hyper_name_finale with compare_token.text.lower()
                                            sim = w2v_model.similarity(hyper_name_final, compare_token.text.lower())
                                        except KeyError:
                                            sim = 0
                                        if sim:
                                            scoring = scoring + abs(sim)

                            scoring_dict[hyper] = scoring / num_cmp
                ...
                if token.text.lower() in fix_variations.keys():
                    ...
                    if max != 0:  # you found the best hyp
                        best_hyp_name_final = from_synset_to_string(best_hyper)
